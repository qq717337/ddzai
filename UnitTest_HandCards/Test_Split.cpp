...
namespace UnitTest_Split
{
	...
	public:
		...
		TEST_METHOD(TestGameTable)
		{
			...
			opt->PlayerCardSet[0] = new HandCards(CardVector{ 0x17,0x27,0x3c,0x2c });
			opt->PlayerCardSet[1] = new HandCards(CardVector{ 0x3b,0x2b });
			opt->PlayerCardSet[2] = new HandCards(CardVector{ 0x1c,0x2c });
			...
			auto thisStyle = table.Play(curPlayIdentity);
		...
		TEST_METHOD(TestNewGame)
		{
			...
			//CardSet *opt=new CardSet();
			// opt->PlayerCardSet[0] = new HandCards(CardVector{ 0x13,0x23,0x43,0x14,0x44,0x45,0x46,0x47,0x28,0x19,0x29,0x39,0x1a,0x1c,0x3c,0x4c,0x1d,0x1e,0x2e,0x4e });
			// opt->PlayerCardSet[1] = new HandCards(CardVector{ 0x33,0x36,0x17,0x48,0x49,0x2a,0x3a,0x4a,0x1b,0x2d,0x3d,0x4d,0x3e,0x1f,0x2f,0x3f,0x1 });
			// opt->PlayerCardSet[2] = new HandCards(CardVector{ 0x24,0x34,0x15,0x25,0x35,0x16,0x26,0x27,0x37,0x18,0x38,0x2b,0x3b,0x4b,0x2c,0x4f,0x2 });
