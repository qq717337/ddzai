...
namespace Common {
	...
	template<typename T>
	inline static std::string ArrayToString(const T* arr, int64_t n, char delimiter) {
		if (n <= 0) {
			return std::string("");
		}
	...
	template<typename T>
	inline static std::string ArrayClassToString(const T* arr, int64_t n, char delimiter) {
		if (n <= 0) {
			return std::string("");
		}
		std::stringstream str_buf;
		str_buf << (std::string) arr[0];
		for (int64_t i = 1; i < n; ++i) {
			str_buf << delimiter;
			str_buf << (std::string) arr[i];
		}
		return str_buf.str();
	}
	...
	/* the class should override the operator std::sting() const */
	template<typename T>
	inline static std::string ArrayClassToString(std::vector<T> arr, char delimiter) {
		if (arr.size() <= 0) {
			return std::string("");
		}
		std::stringstream str_buf;
