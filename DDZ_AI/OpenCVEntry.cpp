...
void OpenCVEntry::Show(const cv::String& img1, const cv::String& img2)
{
	...
	auto mat1 = imread(img1);
	auto mat2 = imread(img2);
...
const Mat OpenCVEntry::GetCardImage(uint8_t cardValue, bool rotate)
{
	if (rotate) {
		Mat src = image_map[GetCardName(cardValue)];

		Mat srcCopy(cvSize(src.rows, src.cols), src.depth(), src.channels());
